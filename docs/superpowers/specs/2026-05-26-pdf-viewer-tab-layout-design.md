...
`pageCount` defaults from `fm.physical?.pages_total ?? fm.physical?.page_count`. When unknown, the tab label degrades to "Read full PDF" (no count).
...
**Below the tabs (always visible):** unchanged from today. Themes chips, Tier-B disclaimer, `<PeopleInPiece />`, `<RelatedSection />`.
...
if (tablist) {
  ...
  function activate(idx, { syncHash = true } = {}) {
    tabs.forEach((t, i) => {
  ...
  if (initial !== 0) {
    activate(initial, { syncHash: false });
  }
}
```
...
Tab nav uses the existing design tokens (`--color-saffron-700` for active accent, `--color-border` for the underline, `--font-ui` for tab labels). Sketch:
...
<div class="border-b border-(--color-border) mb-8 flex font-(family-name:--font-ui)">
  ...
  <button role="tab" class="...same classes...">
    Read full PDF · {pageCount} pages
  </button>
</div>
```
...
- Open a known good slug (e.g. `a-blueprint-for-eradication-of-poverty-dr-b-p-godrej-december-15-1980`).
...
- Open the same slug with `#read` in the URL → PDF tab is active on initial render; iframe is in DOM.
- Open a slug without `pdf_url` (e.g. `aandolan`) → no tab nav; layout matches the current production page.
