---
id: 661-agrarian-organisational-pattern
title:
  main: "AGRARIAN ORGANISATIONAL PATTERN"
  subtitle: ""
work_type: resolution
work_type_source: summary
authors: []
editors: []
contributors: []
related_thinkers: []
publication:
  language: en
  issuer_id: indian-national-congress
  place: "Nagpur"
  year: 1959
provenance:
  source: ccs_archive
  scan_quality: fair
  notes: "llm-extract v1.5; metadata.a canonical, metadata.b cross-checked"
physical:
  pages_total: 2
  pages_total_source: pypdfium2
pdf_url: https://archive.indianliberals.in/swatantra-party-papers/661-agrarian-organisational-pattern.pdf
cover_image: "https://archive.indianliberals.in/covers/661-agrarian-organisational-pattern.webp"
rights:
  status: takedown_on_request
  license: in-copyright
  license_url: null
  rights_statement: Rights held by original depositors / Centre for Civil Society; reproduced for archival access.
themes: []
proposed_themes:
  - "agricultural-reform"
  - "land-reform"
  - "property-rights"
  - "economic-liberty"
  - "public-sector-critique"
missing_metadata_flags:
  - "author_not_named"
  - "publisher_not_named"
  - "title_page_not_found"
  - "no_publisher_address"
  - "no_toc"
authors_resolution:
  method: vision
  confidence: medium
  proposed_unknowns: []
needs_review: false
draft: false
ai:
  drafted_by: claude-sonnet-4.5
  model_version: v1.5
summary: |-
  This two-page English-language document reproduces a resolution on agrarian organisation passed by the Indian National Congress at its 64th session in Nagpur on January 10, 1959. It proposes village development through empowered village panchayats and cooperatives, with cooperatives providing credit, marketing, storage, improved cultivation techniques, animal husbandry, fisheries, and cottage industries. Membership is to be open to all permanent village residents, whether landowners or not.
  
  The resolution identifies cooperative joint farming as the desired future pattern: land would be pooled for joint cultivation while farmers retained their property rights and received net produce in proportion to their land. It proposes service cooperatives as a preliminary step, land ceilings and abolition of intermediaries by the end of 1959, minimum crop prices fixed before sowing, direct government purchases where necessary, state trading in wholesale foodgrains, and the cultivation of unused and wasteland. The document concludes by calling on governments and the Congress organisation to mobilise farmers through mass enthusiasm, initiative, and self-help.
---

## Summary

This two-page English-language document reproduces a resolution on agrarian organisation passed by the Indian National Congress at its 64th session in Nagpur on January 10, 1959. It proposes village development through empowered village panchayats and cooperatives, with cooperatives providing credit, marketing, storage, improved cultivation techniques, animal husbandry, fisheries, and cottage industries. Membership is to be open to all permanent village residents, whether landowners or not.

The resolution identifies cooperative joint farming as the desired future pattern: land would be pooled for joint cultivation while farmers retained their property rights and received net produce in proportion to their land. It proposes service cooperatives as a preliminary step, land ceilings and abolition of intermediaries by the end of 1959, minimum crop prices fixed before sowing, direct government purchases where necessary, state trading in wholesale foodgrains, and the cultivation of unused and wasteland. The document concludes by calling on governments and the Congress organisation to mobilise farmers through mass enthusiasm, initiative, and self-help.

## Key points

- Village organisation should rest on adequately empowered panchayats and village cooperatives.
- Cooperatives should support credit, marketing, storage, improved farming, animal husbandry, fisheries, and cottage industries.
- Cooperative joint farming is proposed, with farmers retaining property rights and receiving shares of net produce in proportion to their land.
- Service cooperatives should be established throughout the country within three years as a preliminary step.
- Land ceilings, abolition of intermediaries, and related land-reform legislation should be completed in all states by the end of 1959.
- The resolution calls for advance minimum prices, direct crop purchases where necessary, state trading in wholesale foodgrains, and the cultivation of unused and wasteland.
- Implementation should combine government action with Congress-led mobilisation, farmer initiative, and self-help.
