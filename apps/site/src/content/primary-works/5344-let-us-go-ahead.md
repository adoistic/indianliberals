---
id: 5344-let-us-go-ahead
title:
  main: "LET US GO AHEAD"
  subtitle: "(IS NOT THE AGITATION FOR LINGUISTIC PROVINCES REACTIONARY AND WILL IT NOT PUT THE CLOK BACK-WARDS?)"
work_type: pamphlet
authors: []
editors: []
contributors: []
related_thinkers: []
publication:
  language: en
  place: "Jinthri, via Song."
  year: 1958
provenance:
  source: ccs_archive
  scan_quality: fair
  notes: "llm-extract v1.5; metadata.a canonical, metadata.b cross-checked"
physical:
  pages_total: 12
  pages_total_source: pypdfium2
pdf_url: https://archive.indianliberals.in/swatantra-party-papers/5344-let-us-go-ahead.pdf
rights:
  status: takedown_on_request
  license: in-copyright
  license_url: null
  rights_statement: Rights held by original depositors / Centre for Civil Society; reproduced for archival access.
themes: []
proposed_themes:
  - "federalism"
  - "economic-liberty"
  - "party-politics"
  - "governance-reform"
  - "indian-liberal-history"
  - "linguistic-reorganisation"
missing_metadata_flags:
  - "publisher_not_printed"
  - "no_title_page"
  - "no_formal_toc"
authors_resolution:
  method: vision
  confidence: medium
  proposed_unknowns: []
needs_review: true
draft: false
ai:
  drafted_by: claude-sonnet-4.5
  model_version: v1.5
summary: |-
  This 12-page pamphlet, signed by J. R. Manjeri and dated October 1958, argues against the agitation for linguistic provinces, particularly the division of Bombay State into Maha-Gujarat and Samyukta Maharashtra. It acknowledges the grief, anger, and violence surrounding the August 1956 decision to retain a bilingual Bombay State, but contends that Parliament’s decision prevented a more dangerous national crisis. The pamphlet presents linguistic reorganisation as, at best, a temporary patch that leaves underlying political and economic problems unresolved.
  
  The argument locates the controversy in competing economic interests, especially Bombay City’s role as an employment and industrial centre for Maharashtrians, Gujaratis, Madrasis, and others. Manjeri criticises political leaders for mobilising legitimate regional grievances through the language issue, and urges intellectuals to explain the economic causes of the conflict, promote industrial development and fair distribution, and help people understand their shared national interests. The pamphlet closes with an appeal to leaders to put India first, reject linguistic chauvinism, and seek a larger national settlement rather than “bigger and bigger zones.”
---

## Key points

- The pamphlet defends the 1956 decision to retain a bilingual Bombay State.
- It argues that linguistic provincialism risks intensifying conflict between neighbouring states.
- It identifies economic interests, employment patterns, and Bombay City’s industrial importance as central causes of the dispute.
- It warns that language-based agitation may obscure deeper economic grievances and reproduce the dangers that preceded Partition.
- It criticises leaders for exploiting regional dissatisfaction and patriotic sentiment for political ends.
- It assigns the intelligentsia a responsibility to explain economic and social realities to the public.
- It advocates industrial development, fair distribution, national solidarity, and a politics that places India before regional claims.
