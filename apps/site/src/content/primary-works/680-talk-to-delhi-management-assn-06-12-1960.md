---
id: 680-talk-to-delhi-management-assn-06-12-1960
title:
  main: "Talk to Delhi Management Assn."
  subtitle: ""
work_type: speech
authors: []
editors: []
contributors: []
related_thinkers: []
publication:
  language: en
  year: 1960
provenance:
  source: ccs_archive
  scan_quality: fair
  notes: "llm-extract v1.5; metadata.a canonical, metadata.b cross-checked"
physical:
  pages_total: 5
  pages_total_source: pypdfium2
pdf_url: https://archive.indianliberals.in/swatantra-party-papers/680-talk-to-delhi-management-assn-06-12-1960.pdf
rights:
  status: takedown_on_request
  license: in-copyright
  license_url: null
  rights_statement: Rights held by original depositors / Centre for Civil Society; reproduced for archival access.
themes: []
proposed_themes:
  - "education"
  - "governance-reform"
  - "economic-liberty"
missing_metadata_flags:
  - "author_not_printed"
  - "publisher_not_printed"
  - "place_not_printed"
  - "delivery_venue_not_printed"
  - "delivery_date_not_printed_separately"
authors_resolution:
  method: vision
  confidence: medium
  proposed_unknowns: []
needs_review: true
draft: false
ai:
  drafted_by: claude-sonnet-4.5
  model_version: v1.5
summary: |-
  This five-page typescript outline records a talk to the Delhi Management Association on 6 December 1960 about recruitment and management selection. It argues that hiring is a substantial investment: a poor appointment can generate costs through mistakes, wasted training, lost time, and lack of organisational harmony. The outline contrasts hurried or intuitive recruitment with more systematic selection, while warning against replacing judgment with mechanical testing or bureaucratic attitudes.
  
  The speaker proposes assessing candidates against the requirements of a specific job, including technical qualifications, skills and aptitudes, mental ability, and physical fitness. The process outlined includes a comprehensive application form, a sufficiently large candidate field, reference checks, interviews and tests, induction, placement, probation, appraisal, career planning, and training. The final section stresses specialisation and skilled observation, concluding that appraisal of people is "more an art than a science." The pages do not identify the speaker, publisher, venue beyond the reference to the Delhi Management Association, or the sources of the cited examples.
---

## Summary

This five-page typescript outline records a talk to the Delhi Management Association on 6 December 1960 about recruitment and management selection. It argues that hiring is a substantial investment: a poor appointment can generate costs through mistakes, wasted training, lost time, and lack of organisational harmony. The outline contrasts hurried or intuitive recruitment with more systematic selection, while warning against replacing judgment with mechanical testing or bureaucratic attitudes.

The speaker proposes assessing candidates against the requirements of a specific job, including technical qualifications, skills and aptitudes, mental ability, and physical fitness. The process outlined includes a comprehensive application form, a sufficiently large candidate field, reference checks, interviews and tests, induction, placement, probation, appraisal, career planning, and training. The final section stresses specialisation and skilled observation, concluding that appraisal of people is "more an art than a science." The pages do not identify the speaker, publisher, venue beyond the reference to the Delhi Management Association, or the sources of the cited examples.

## Key points

- Recruitment is presented as an investment whose failure can cost an organisation heavily in salaries, training, travel, and administrative work.
- The outline criticises hurried, arbitrary, and appearance-based selection practices.
- Psychological tests and other instruments are described as aids to judgment rather than substitutes for judgment.
- Selection must fit the person to a specific job; an employee successful in one position may fail in another.
- Internal recruitment is treated as broadly equivalent to promotion, but with reservations about automatic advancement.
- A sound recruitment process should include applications, a broad candidate pool, references, interviews, induction, probation, appraisal, career planning, and training.
- The assessment of executives is characterised as a skilled practical activity that combines observation with formal techniques.
