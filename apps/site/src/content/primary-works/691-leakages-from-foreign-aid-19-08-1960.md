---
id: 691-leakages-from-foreign-aid-19-08-1960
title:
  main: "Leakages From Foreign Aid"
  subtitle: ""
work_type: essay
authors:
  - b-r-shenoy
editors: []
contributors: []
related_thinkers: []
publication:
  language: en
  publisher_name: "THE HIND"
  year: 1960
  year_source: filename
provenance:
  source: ccs_archive
  scan_quality: fair
  notes: "llm-extract v1.5; metadata.a canonical, metadata.b cross-checked"
physical:
  pages_total: 3
  pages_total_source: pypdfium2
pdf_url: https://archive.indianliberals.in/swatantra-party-papers/691-leakages-from-foreign-aid-19-08-1960.pdf
rights:
  status: takedown_on_request
  license: in-copyright
  license_url: null
  rights_statement: Rights held by original depositors / Centre for Civil Society; reproduced for archival access.
themes:
  - fiscal-policy
proposed_themes:
  - "planning-critique"
  - "foreign-policy"
  - "economic-liberty"
  - "public-sector-critique"
  - "regulatory-state-critique"
  - "anti-corruption"
missing_metadata_flags:
  - "year_not_printed"
  - "publisher_not_fully_identified"
  - "place_not_printed"
  - "no_toc"
authors_resolution:
  method: vision
  confidence: high
  proposed_unknowns: []
needs_review: true
draft: false
ai:
  drafted_by: claude-sonnet-4.5
  model_version: v1.5
summary: |-
  In the rendered pages, B. R. Shenoy argues that foreign aid is being absorbed inefficiently within India’s Third Plan. He contends that the Plan’s investment requirements exceed available domestic savings and foreign assistance, while inflation, import restrictions, and exchange-control distortions weaken the productive value of aid. He contrasts the Third Plan’s assumptions with the experience of the Second Plan, citing large reserve losses, payment deficits, and the diversion of aid toward consumption and inventories rather than productive investment.
  
  In the rendered pages, Shenoy identifies several channels through which aid “leaks”: over-expansion of the public sector, under-utilised industrial and irrigation capacity, import licensing, over-invoicing and under-invoicing, and corruption. He argues that public-sector investment has generated insufficient returns and that foreign aid may end up financing the foreign-exchange counterpart of domestic savings that have been misdirected. The article therefore presents aid not as a substitute for sound fiscal and monetary management, but as a resource whose benefits are undermined by planning, controls, waste, and weak administrative accountability.
---

## Key points

- Shenoy argues that the Third Plan’s investment targets exceed the resources available from domestic savings and foreign aid.
- The article links foreign-exchange shortages and payment deficits to inflation, import controls, and inefficient investment.
- Second Plan experience is used to show reserve depletion, rising debt-service pressures, and a widening balance-of-payments problem.
- Aid-funded imports are said to include consumption goods and other items that do not directly expand productive capacity.
- The article criticises public-sector expansion, pointing to under-utilised industrial and irrigation capacity.
- Import licensing, fictitious resale margins, over-invoicing, and under-invoicing are presented as mechanisms that divert or waste aid.
- Shenoy warns that public-sector expansion creates conditions favourable to corruption and easy spending.
