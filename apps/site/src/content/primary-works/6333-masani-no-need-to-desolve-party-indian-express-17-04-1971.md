---
id: 6333-masani-no-need-to-desolve-party-indian-express-17-04-1971
title:
  main: "Masani: No need to dissolve party"
  subtitle: ""
work_type: essay
authors: []
editors: []
contributors: []
related_thinkers: []
publication:
  language: en
  publisher_name: "INDIAN EXPRESS"
  year: 1971
provenance:
  source: ccs_archive
  scan_quality: fair
  notes: "llm-extract v1.5; metadata.a canonical, metadata.b cross-checked"
physical:
  pages_total: 1
  pages_total_source: pypdfium2
pdf_url: https://archive.indianliberals.in/swatantra-party-papers/6333-masani-no-need-to-desolve-party-indian-express-17-04-1971.pdf
cover_image: "https://archive.indianliberals.in/covers/6333-masani-no-need-to-desolve-party-indian-express-17-04-1971.webp"
rights:
  status: takedown_on_request
  license: in-copyright
  license_url: null
  rights_statement: Rights held by original depositors / Centre for Civil Society; reproduced for archival access.
themes: []
proposed_themes:
  - "party-politics"
  - "constitutionalism"
missing_metadata_flags:
  - "no_title_page"
  - "no_publisher_address"
  - "publication_venue_not_fully_identified"
  - "clipping_contains_second_article"
authors_resolution:
  method: vision
  confidence: medium
  proposed_unknowns: []
needs_review: true
draft: false
ai:
  drafted_by: claude-sonnet-4.5
  model_version: v1.5
summary: |-
  This 17 April 1971 Indian Express clipping reports that M. R. Masani argued there was no need to dissolve the Swatantra Party, despite the party's poor showing in the recent elections. At an executive meeting in Bombay, Masani reportedly said the party still had an important role to play and that its immediate task was to reassess its position and determine how it could perform a constructive role in the present situation. The report also notes plans for an ad hoc committee to examine the party's future and review its current political position.
  
  A second short article, headed “Rajaji sad,” reports C. Rajagopalachari's disappointment with the party's electoral outcome. Rajagopalachari is quoted as saying that attempts to bring a consolidated democratic opposition into existence had failed, and that the Swatantra Party's mission remained to fight against totalitarian tendencies and defend the Constitution.
---

## Summary

This 17 April 1971 Indian Express clipping reports that M. R. Masani argued there was no need to dissolve the Swatantra Party, despite the party's poor showing in the recent elections. At an executive meeting in Bombay, Masani reportedly said the party still had an important role to play and that its immediate task was to reassess its position and determine how it could perform a constructive role in the present situation. The report also notes plans for an ad hoc committee to examine the party's future and review its current political position.

A second short article, headed “Rajaji sad,” reports C. Rajagopalachari's disappointment with the party's electoral outcome. Rajagopalachari is quoted as saying that attempts to bring a consolidated democratic opposition into existence had failed, and that the Swatantra Party's mission remained to fight against totalitarian tendencies and defend the Constitution.

## Key points

- M. R. Masani rejected the view that the Swatantra Party needed to be dissolved after its electoral setback.
- Masani described the party as continuing to have an important role in Indian politics.
- The party executive planned to review its position and consider how it could play a constructive role.
- An ad hoc committee was proposed to examine the party's future.
- C. Rajagopalachari expressed sadness that efforts to create a consolidated democratic opposition had failed.
- Rajagopalachari reaffirmed the Swatantra Party's stated mission of opposing totalitarian tendencies and defending the Constitution.
