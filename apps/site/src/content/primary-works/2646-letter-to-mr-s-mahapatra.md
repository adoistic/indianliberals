---
id: 2646-letter-to-mr-s-mahapatra
title:
  main: "AN APPEAL"
  subtitle: "TO THE CHIEF MINISTER & THE MINISTER OF EDUCATION, ORISSA"
work_type: letter
authors: []
editors: []
contributors: []
related_thinkers: []
publication:
  language: en
  publisher_name: "Pathya Pustak Prakasak Samiti"
  place: "Cuttack-2"
provenance:
  source: ccs_archive
  scan_quality: fair
  notes: "llm-extract v1.5; metadata.a canonical, metadata.b cross-checked"
physical:
  pages_total: 12
  pages_total_source: pypdfium2
pdf_url: https://archive.indianliberals.in/swatantra-party-papers/2646-letter-to-mr-s-mahapatra.pdf
rights:
  status: takedown_on_request
  license: in-copyright
  license_url: null
  rights_statement: Rights held by original depositors / Centre for Civil Society; reproduced for archival access.
themes:
  - free-enterprise
proposed_themes:
  - "education"
  - "regulatory-state-critique"
  - "public-sector-critique"
  - "economic-liberty"
  - "literature"
missing_metadata_flags:
  - "year_not_printed"
  - "no_named_author"
  - "publisher_not_resolved"
  - "issuer_not_resolved"
  - "editor_not_named"
  - "no_formal_toc"
authors_resolution:
  method: vision
  confidence: medium
  proposed_unknowns: []
needs_review: true
draft: false
ai:
  drafted_by: claude-sonnet-4.5
  model_version: v1.5
summary: |-
  This appeal, introduced by a covering letter from S. Mahapatra to Minoo Masani, asks the Chief Minister and Education Minister of Orissa to withdraw the state monopoly over school-textbook production. Issued collectively by Orissa’s writers, publishers and printers, it argues that the policy was imposed without meaningful consultation with educators, violates democratic and civilised principles, and threatens the state’s educational culture, regional-language literature and publishing economy.
  
  The appeal presents the policy as both an administrative failure and an economic injustice. It claims that nearly 100 privately owned printing presses, binding workshops and type foundries had closed or were closing, leaving about 3,000 skilled workers unemployed. The writers and publishers also argue that state-produced textbooks were neither cheaper nor better, while private publishers had historically subsidised Oriya literature through textbook revenues. Drawing on press criticism and comparisons with other countries, including communist states that had moved away from rigid state monopolies, the appeal concludes with a prayer for the restoration of private participation in textbook production in the name of democracy, free enterprise, education and the future of Oriya literature.
---

## Summary

This appeal, introduced by a covering letter from S. Mahapatra to Minoo Masani, asks the Chief Minister and Education Minister of Orissa to withdraw the state monopoly over school-textbook production. Issued collectively by Orissa’s writers, publishers and printers, it argues that the policy was imposed without meaningful consultation with educators, violates democratic and civilised principles, and threatens the state’s educational culture, regional-language literature and publishing economy.

The appeal presents the policy as both an administrative failure and an economic injustice. It claims that nearly 100 privately owned printing presses, binding workshops and type foundries had closed or were closing, leaving about 3,000 skilled workers unemployed. The writers and publishers also argue that state-produced textbooks were neither cheaper nor better, while private publishers had historically subsidised Oriya literature through textbook revenues. Drawing on press criticism and comparisons with other countries, including communist states that had moved away from rigid state monopolies, the appeal concludes with a prayer for the restoration of private participation in textbook production in the name of democracy, free enterprise, education and the future of Oriya literature.

## Key points

- The document appeals against the nationalisation or state monopolisation of school-textbook production in Orissa.
- The policy is portrayed as having been introduced abruptly and without adequate consultation with educationists.
- The appeal links the state monopoly to the closure of printing and publishing businesses and unemployment among approximately 3,000 skilled workers.
- It argues that state-produced textbooks have not demonstrated superior quality or lower prices.
- Private publishers are presented as having supported Oriya literature by using textbook income to finance literary publishing.
- The document invokes democracy, free enterprise, educational standards and cultural preservation in support of its request.
- The signatories ask the government to withdraw the measure and allow private publishers, printers and writers to resume their role.
