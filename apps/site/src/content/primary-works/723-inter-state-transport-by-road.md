---
id: 723-inter-state-transport-by-road
title:
  main: "INTER-STATE TRANSPORT BY ROAD"
  subtitle: ""
work_type: essay
authors:
  - minoo-masani
editors: []
contributors: []
related_thinkers: []
publication:
  language: en
  publisher_name: "Reproduced from Economic Times"
provenance:
  source: ccs_archive
  scan_quality: fair
  notes: "llm-extract v1.5; metadata.a canonical, metadata.b cross-checked"
physical:
  pages_total: 3
  pages_total_source: pypdfium2
pdf_url: https://archive.indianliberals.in/swatantra-party-papers/723-inter-state-transport-by-road.pdf
cover_image: "https://archive.indianliberals.in/covers/723-inter-state-transport-by-road.webp"
rights:
  status: takedown_on_request
  license: in-copyright
  license_url: null
  rights_statement: Rights held by original depositors / Centre for Civil Society; reproduced for archival access.
themes: []
proposed_themes:
  - "regulatory-state-critique"
  - "public-sector-critique"
  - "economic-liberty"
  - "planning-critique"
  - "federalism"
missing_metadata_flags:
  - "year_not_printed"
  - "publisher_not_identified"
  - "place_not_printed"
  - "no_toc"
authors_resolution:
  method: vision
  confidence: high
  proposed_unknowns: []
needs_review: false
draft: false
ai:
  drafted_by: claude-sonnet-4.5
  model_version: v1.5
summary: |-
  In the rendered pages, Minoo Masani argues that inter-State road transport is being crippled by a dense network of restrictions, permits, taxes, weight limits, border checks, and administrative delays. He traces these obstacles to the Motor Vehicles Act of 1939 and subsequent policies intended to protect the Railways from so-called “unhealthy” competition. The article criticises the Government for giving the Inter-State Transport Commission inadequate powers and for allowing State governments to treat inter-State traffic as a marginal concern, except when tax revenue is involved.
  
  In the rendered pages, Masani presents road transport as essential to closing an expanding national transport-capacity gap. He cites the inadequacy of the existing permit system, multiple taxation of consignments, inconsistent truck weight limits, bridge and road deficiencies, river-crossing detours, and octroi posts as practical impediments to economic expansion. He calls for the Commission to receive effective powers and to issue permits freely when additional demand is established, while arguing that a stronger road network would relieve pressure on the Railways and enable them to serve defence-related industries and neglected passengers and customers.
---

## Summary

In the rendered pages, Minoo Masani argues that inter-State road transport is being crippled by a dense network of restrictions, permits, taxes, weight limits, border checks, and administrative delays. He traces these obstacles to the Motor Vehicles Act of 1939 and subsequent policies intended to protect the Railways from so-called “unhealthy” competition. The article criticises the Government for giving the Inter-State Transport Commission inadequate powers and for allowing State governments to treat inter-State traffic as a marginal concern, except when tax revenue is involved.

In the rendered pages, Masani presents road transport as essential to closing an expanding national transport-capacity gap. He cites the inadequacy of the existing permit system, multiple taxation of consignments, inconsistent truck weight limits, bridge and road deficiencies, river-crossing detours, and octroi posts as practical impediments to economic expansion. He calls for the Commission to receive effective powers and to issue permits freely when additional demand is established, while arguing that a stronger road network would relieve pressure on the Railways and enable them to serve defence-related industries and neglected passengers and customers.

## Key points

- Masani describes inter-State road transport as a field dominated by restrictions and administrative handicaps.
- The Motor Vehicles Act of 1939 is presented as a product of efforts to shield the Railways from road competition.
- State-controlled permits are portrayed as arbitrary, insufficient, and obstructive to regional economic progress.
- Separate State taxation causes the same road consignment to be taxed repeatedly.
- Different State weight limits, poor bridges and roads, river-crossing problems, and octroi posts increase costs and delays.
- The article warns that transport capacity is falling dangerously short of the requirements of economic planning and production.
- Masani recommends vesting the Inter-State Transport Commission with sufficient powers to grant permits when demand warrants them.
- Expanded long-distance road transport is presented as complementary to, rather than destructive of, the Railways.
