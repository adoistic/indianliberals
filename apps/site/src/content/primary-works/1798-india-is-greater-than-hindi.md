---
id: 1798-india-is-greater-than-hindi
title:
  main: "INDIA IS GREATER THAN HINDI"
  subtitle: "A CASE FOR THE CONTINUANCE OF THE ENGLISH LANGUAGE"
work_type: essay
authors: []
editors: []
contributors: []
related_thinkers: []
publication:
  language: en
  publisher_name: "MATHAI'S PUBLICATIONS"
  place: "BOMBAY 7"
provenance:
  source: ccs_archive
  scan_quality: fair
  notes: "llm-extract v1.5; metadata.a canonical, metadata.b cross-checked"
physical:
  pages_total: 3
  pages_total_source: pypdfium2
pdf_url: https://archive.indianliberals.in/swatantra-party-papers/1798-india-is-greater-than-hindi.pdf
cover_image: "https://archive.indianliberals.in/covers/1798-india-is-greater-than-hindi.webp"
rights:
  status: takedown_on_request
  license: in-copyright
  license_url: null
  rights_statement: Rights held by original depositors / Centre for Civil Society; reproduced for archival access.
themes: []
proposed_themes:
  - "education"
  - "language-policy"
  - "nationalism-debate"
missing_metadata_flags:
  - "year_not_printed"
  - "publisher_not_resolved"
  - "author_not_resolved"
  - "date_not_printed"
  - "signature_not_visible"
authors_resolution:
  method: vision
  confidence: medium
  proposed_unknowns: []
needs_review: true
draft: false
ai:
  drafted_by: claude-sonnet-4.5
  model_version: v1.5
summary: |-
  Isaac Mathai's essay argues that English should continue as India's common and international language rather than being displaced by Hindi. He presents English as a valuable inheritance of two centuries, linking Indians to accumulated knowledge, science, education, and communication across a country divided by regional languages, religions, and communities. The essay portrays the choice as one affecting future generations and criticizes politicians who, in Mathai's view, disregard the preferences of educators, universities, parents, and other institutions for short-term political advantage.
  
  Mathai challenges the claim that Hindi represents a national majority, emphasizing that most Indians speak regional languages and that Hindi itself has competing forms, including Sanskritised, Persianised, colloquial, and Urdu varieties. He also makes a practical case: replacing English would make higher education and specialized technical knowledge difficult and costly to produce in regional languages. The essay concludes by connecting English with internationalism and calls for English-language societies and educational centres throughout India. The accompanying covering letter solicits paper or cash contributions for a wider circulation among educators, legislators, municipal councillors, and other public institutions.
---

## Summary

Isaac Mathai's essay argues that English should continue as India's common and international language rather than being displaced by Hindi. He presents English as a valuable inheritance of two centuries, linking Indians to accumulated knowledge, science, education, and communication across a country divided by regional languages, religions, and communities. The essay portrays the choice as one affecting future generations and criticizes politicians who, in Mathai's view, disregard the preferences of educators, universities, parents, and other institutions for short-term political advantage.

Mathai challenges the claim that Hindi represents a national majority, emphasizing that most Indians speak regional languages and that Hindi itself has competing forms, including Sanskritised, Persianised, colloquial, and Urdu varieties. He also makes a practical case: replacing English would make higher education and specialized technical knowledge difficult and costly to produce in regional languages. The essay concludes by connecting English with internationalism and calls for English-language societies and educational centres throughout India. The accompanying covering letter solicits paper or cash contributions for a wider circulation among educators, legislators, municipal councillors, and other public institutions.

## Key points

- English is presented as an inherited resource containing accumulated wisdom, knowledge, and experience.
- The essay argues that India's linguistic diversity makes a single Hindi-based national language impractical.
- Mathai disputes the characterization of Hindi speakers as a majority of India's population.
- The text criticizes political efforts to replace English despite support for its continued use among educators and institutions.
- The essay argues that regional-language higher education would be expensive, difficult to maintain, and quickly outdated in specialized fields.
- English is defended as a medium of communication across India's diverse communities and as a bridge to internationalism.
- The covering letter seeks financial or material support to distribute the essay and promote English-language education.
