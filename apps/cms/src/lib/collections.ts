...
export type FieldKind =
  ...
  | 'object' | 'object-list'
  | 'image';           // a picture, dropped on the form rather than typed as a path

/**
 * Where a dropped picture ends up.
 *
 * `r2` pictures go straight to the archive bucket and the field holds the
 * full web address, which is how work covers have always been stored.
 * `repo` pictures are committed into the site repository beside the entry
 * (staged in the bucket until the save), and the field holds the site path,
 * which is how portraits, logos and hero images have always been stored.
 */
export interface ImageTarget {
  store: 'r2' | 'repo';
  /** For repo pictures: the public/ directory they live in, e.g. /thinkers/photos. */
  dir?: string;
}
...
export interface Field {
  ...
  image?: ImageTarget;    // for image fields: where a dropped picture goes
