<!-- v1.2 — v1.3 patch: D8 page_system consistency-within-essay rule, attribution-by-speaker rule for cross-thinker mentions. v1.1 baseline: D2 kebab-case themes, D3 cross-script Indic resolver, D4 editor-preview ban, D5 extent_caveat, D8 page_system on pull_quotes + cross_thinker_mentions, D9 transcription_anomaly, D10 recommended_authority_additions, essay_focused mode (continuation loop), PREV_SUBCHUNK_CONTEXT support. -->
...
**`page_system` consistency rule (D8).** `page_system` MUST stay consistent across ALL `pull_quotes` and ALL `cross_thinker_mentions` within a single `essays_summarized[]` entry. Pick one of `"printed"` or `"pdf"` based on what's visible on the essay's first rendered page, and use it for every quote and mention in that essay. The only legitimate exception is when printed folios are *visibly absent* on some pages of the same essay (a known issue with annexures and appendices in proceedings volumes) — in that case, set `page_system: "pdf"` for the entire essay's quotes and mentions, AND record a one-line explanation in the first quote's optional `page_system_note` field (e.g., `"folios absent from p.86 onward"`). Do NOT silently flip from `"printed"` to `"pdf"` mid-essay.
...
**Attribution-by-speaker rule.** A cross-thinker mention belongs to the essay whose author is *speaking*, not to the essay whose `toc_index` range covers the printed-page number. When a rendered page sits at the boundary between two essays (last page of essay A is the first page rendered for essay B's chunk, or vice versa), attribute each mention to the essay whose body text introduced it. Concretely: if the bottom half of page 39 carries the closing paragraph of von Welck's address and the top half of page 40 starts Raju's address, a Kennedy quote in von Welck's closing paragraph belongs in toc_index 4 (von Welck), NOT toc_index 5 (Raju), even when chunk 5 is the chunk that rendered page 39. If your rendered chunk straddles a boundary, omit mentions from the non-target essay entirely and let the chunk for the speaking essay capture them.
