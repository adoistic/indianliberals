...
**Rule 9 EXCEPTION — periodical mastheads.** When the work is a periodical (`work_type: "periodical_issue"`), the masthead inside the front matter (typically printed on the inside cover or page 2) is a *content* element, not a colophon. A masthead listing of the form `"Editor: <Name>"` or `"Editor-in-Chief: <Name>"` (without an accompanying "Published by:" verb tying it to issuance) IS explicit editorial credit and MUST be captured in `editors[]`. The Rule 9 ban applies ONLY to publisher-signature lines on colophons / copyright pages / back covers — not to magazine mastheads. If the same person appears in both the masthead (as editor) and the colophon (as issuing signatory), capture them as the editor and note the dual role in `editors[0].notes`. When in doubt, the test is: does the rendered page treat this name as a content credit (above the fold, in a content header) or as a publication-machinery imprint (in fine print, with the publisher's address)? Content credit → `editors[]`. Machinery imprint → `publisher_verbatim` only.
